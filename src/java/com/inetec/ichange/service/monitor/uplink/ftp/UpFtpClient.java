...
//        int code = con.upload(file);
        try {
            File f = new File(file);
            InputStream in = new FileInputStream(f);
//            int code = con.upload(file,in);
            int code = con.upload(file,f.getName(),in);
            in.close();
    ...
    public static void main(String argv[]) {
        if (argv.length == 3) {
            ...
            UpFtpClient client = new UpFtpClient();
            client.init("192.168.1.105",21,"172.16.2.6","123456");
            client.upfile("incoming", "F:/stpfilechange.jar");
        }
    }
