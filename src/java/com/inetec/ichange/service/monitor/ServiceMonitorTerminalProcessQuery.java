 ...
 */
public class ServiceMonitorTerminalProcessQuery implements IServiceCommondProcess {
    private static Logger logger = Logger.getLogger(ServiceMonitorTerminalProcessQuery.class);
    /**
     * @param input
    ...
    public DataAttributes process(String fileName, DataAttributes dataAttributes) throws Ex {
        ...
        if (!reportCommand.equalsIgnoreCase("")){
            ...
            try {
                dataAttributes.setResultData(mcclient.processQuery(policeno, ip));
                status = "200";
            } catch (Exception e) {
