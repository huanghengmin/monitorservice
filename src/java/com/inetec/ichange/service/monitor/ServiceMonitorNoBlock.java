 ...
 */
public class ServiceMonitorNoBlock implements IServiceCommondProcess {
    private static Logger logger = Logger.getLogger(ServiceMonitorNoBlock.class);
    ...
    public DataAttributes process(String fileName, DataAttributes dataAttributes) throws Ex {
        ...
        if (!idno.equalsIgnoreCase("")) {
            logger.info("Service monitor  vpn noblock call mc client begin.");
            McHttpClient mcclient = new McHttpClient();
            mcclient.init(new MonitorAgentDao().getMcHost());
            try {
                if (mcclient.vpnnoblock(idno, ip)) {
                    status = "200";
