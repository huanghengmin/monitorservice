 ...
 */
public class ServiceMonitorTerminalNetQuery implements IServiceCommondProcess {
    private static Logger logger = Logger.getLogger(ServiceMonitorTerminalNetQuery.class);
    /**
     * @param input
    ...
    public DataAttributes process(String fileName, DataAttributes dataAttributes) throws Ex {
        ...
        if (!reportCommand.equalsIgnoreCase("")){
            ...
            try {
                dataAttributes.setResultData(mcclient.netQuery(policeno, ip));
                status = "200";
            } catch (Exception e) {
